...
func NewAdapterService(repo *repository.AdapterRepository, pkgRepo *repository.AdapterPackageRepository, sessionRepo *repository.AdapterSessionRepository) *AdapterService {
...
// StartAdapter starts an adapter instance, creating a running session record.
func (s *AdapterService) StartAdapter(id string) error {
	...
	// Upsert a running session for the adapter instance
	session, err := s.sessionRepo.FindByInstanceID(id)
	if err != nil {
		// No existing session, create a new one
		now := time.Now()
		session = &models.AdapterSession{
			ID:            models.GenerateID(),
			InstanceID:    id,
	...
	// Existing session, mark it connected again
	now := time.Now()
	session.Status = "connected"
	session.ConnectedAt = now
	session.LastHeartbeat = now
	session.LeaseExpiry = now.Add(5 * time.Minute)
	return s.sessionRepo.Update(session)
}

// StopAdapter stops an adapter instance, marking its session as disconnected.
func (s *AdapterService) StopAdapter(id string) error {
	session, err := s.sessionRepo.FindByInstanceID(id)
	if err != nil {
		// No session exists; treat as already stopped
		return nil
	}
