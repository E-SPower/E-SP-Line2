...
def generate_mid():
    mid = taobao_js.call('generate_mid')
    return mid

def generate_uuid():
    uuid = taobao_js.call('generate_uuid')
    return uuid

def generate_device_id(user_id):
    device_id = taobao_js.call('generate_device_id', user_id)
    return device_id

def generate_sign(t, token, data):
    sign = taobao_js.call('generate_sign', t, token, data)
    return sign

def decrypt(data):
    res = taobao_js.call('decrypt', data)
    return res
...
if __name__ == '__main__':
    ...
    msg = "gQGRhAHaACgzODg4Nzc3MTA4LjEtMjU5MTU2MDE5Mi4xIzExMDAxQGNudGFvYmFvAgADAQSzMzg4ODc3NzEwOEBjbnRhb2Jhbw=="
